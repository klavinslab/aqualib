...
class Protocol
  ...
  def main
    ...
    io_hash = { debug_mode: "No", gel_band_verify: "No", plate_ids: [], task_ids: [], group: "admin" }.merge io_hash
    debug_mode = false
    ...
    new_stripwells.each_with_index do |stripwell, i|
      ...
      if io_hash[:gel_band_verify].downcase == "yes"
        stripwell_band_verify stripwell, plate_ids: io_hash[:plate_ids]
      end
    end
    ...
    io_hash[:task_ids].each do |tid|
      ...
      task_plate_ids = task.simple_spec[:plate_ids]
      task_plasmid_ids = task_plate_ids.collect { |id| find(:item, id: id)[0].sample.id }
      ...
      new_stripwells.each do |stripwell|
        ...
        if (task_plasmid_ids & stripwell_strain_ids).any?
          associated_stripwell_ids[stripwell.id] = task_plasmid_ids & stripwell_strain_ids
        end
      end
      ...
      associated_stripwell_ids.each do |id, plasmid_ids|
        if !debug_mode
          ...
          plasmid_ids_link = plasmid_ids.collect { |id| item_or_sample_html_link(id, :sample) + " (location: #{Matrix[*gel_matrix].index(id).collect { |i| i + 1}.join(',')})" }.join(", ")
          image_url = "<a href=#{upload_url} target='_blank'>image</a>".html_safe
          notifs.push "#{'Plasmid'.pluralize(plasmid_ids.length)} #{plasmid_ids_link} associated gel: #{item_or_sample_html_link id, :item} (#{image_url}) is uploaded."
        end
      end
    ...
    verification_digest_task_ids.each do |tid|
      ...
      next if stripwell.nil?
      
      upload_id = gel_uploads[stripwell.id][:stripwell][0][:id]
      upload_url = Upload.find(upload_id).url
      gel_matrix = collection_from(stripwell.id).matrix
      image_url = "<a href=#{upload_url} target='_blank'>image</a>".html_safe
      task = find(:task, id: tid)[0]
      template = find(:item, id: task.simple_spec[:template_id])[0]
      template_col = stripwell.datum[:task_id_mapping].index(tid) + 1
      task.notify "#{template.object_type.name} #{item_or_sample_html_link template.id, :item} (location: 1,#{template_col}) associated gel: #{item_or_sample_html_link stripwell.id, :item} (#{image_url}) is uploaded."
    end
