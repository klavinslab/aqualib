    ...
    if io_hash[:task_ids]
      io_hash[:task_ids].each do |tid|
        ...
        if task.simple_spec[:fragments].length == 1
          ...
          produced_fragment_stocks.each { |fragment_stock| notifs.push "This task produces Fragment Stock #{item_or_sample_html_link fragment_stock.id, :item} (conc: #{fragment_stock.datum[:concentration]} ng/uL) for #{sample_html_link fragment_stock.sample}".html_safe }
