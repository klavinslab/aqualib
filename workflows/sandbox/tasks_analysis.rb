...
class Protocol
  ...
  def main
    ...
    correct_tasks, wrong_tasks, redundant_tasks, keep_tasks = [], [], [], []
    done_seq_veri_tasks.each do |t|
      ...
      elsif history.include? "'results back' to 'sequence correct but keep plate'"
        keep_tasks.push t
    ...
    show {
      ...
      note "keep tasks #{keep_tasks.length}, #{keep_tasks.length.to_f/done_seq_veri_tasks.length}"
    }
