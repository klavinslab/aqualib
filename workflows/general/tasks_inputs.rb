...
class Protocol
  ...
  def main
    ...
    if io_hash[:task_name] == "Discard Item"
      divided_yeast_plates_to_delete = items_beyond_days "Divided Yeast Plate", 28
      yeast_plates_to_delete = items_beyond_days "Yeast Plate", 28
    ...
    when "Golden Gate Assembly"
      io_hash = { plasmid_ids: [], backbone_ids: [], inserts_ids: [], restriction_enzyme_ids: [] }.merge io_hash
      io_hash[:task_ids].each do |tid|
        task = find(:task, id: tid)[0]
        io_hash[:plasmid_ids].push task.simple_spec[:plasmid]
