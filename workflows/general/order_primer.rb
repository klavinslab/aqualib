...
class Protocol
  ...
  def main
    ...
    show {
      title "Params"
      note Parameter.get_float("short primer cost")
    }
    ...
    primers_over_90 = primers.select.with_index { |p, idx| primer_lengths[idx] > 90 }
    idt = show {
      title "Create an IDT DNA oligos order"
      warning "Oligo concentration for primer(s) #{primers_over_60} will have to be set to \"100 nmole DNA oligo.\""
      warning "Oligo concentration for primer(s) #{primers_over_90} will have to be set to \"250 nmole DNA oligo.\""
