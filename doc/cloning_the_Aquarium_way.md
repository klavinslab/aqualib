...
---
#### How it works?
The yeast transformation workflow takes a yeast strain id as input and produces a yeast plate through transforming a digested plasmid into a parent yeast strain. The yeast strain sample field defines hte plasmid and the parent yeast strain. The workflow can be scheduled by starting yeast_transformtion metacol. In detail, it pools all the yeast transformation tasks, starts overnights for the parent strains that do not have enough yeast competent cells for this batch of transformation. If there is any overnights, it then progresses to inoculate overnights into large volume growth and make as many as possible yeast competent cells and places in the M80 boxes. In the meantime, it digests the plasmid stock of the plasmid that specified in the plasmid field of the yeast strain. It then transforms the digested plasmids into the parent strain yeast competent cells and plates them on selective media plates using info specified in the yeast marker field of the plasmid. The workflow currently can handle plasmids with KanMX yeast marker, it will incubate the transformed mixtures for 3 hours and then plate on +G418 plates.

#### Input requirements
For each yeast strain entered in the tasks, you need to enter the **Parent** and **Plasmid** info in the sample field. The parent is to link a yeast strain as your parent strain for this transformation and plasmid is the plasmid you are planning to digest and transform into the parent strain to make the yeast transformed strain. You need to make sure there is at least one plasmid stock for the plasmid.

#### How to submit a task?
To submit a yeast transformation task, go to Tasks/Yeast Transformation, click New Yeast Transformation, enter Name as an identifier for you to recognize, could be any string that does not conflict with existing task names under Yeast Transformation task, leave the Status as "waiting for ingredients", enter the sample ids of the yeast strains you want to make.
