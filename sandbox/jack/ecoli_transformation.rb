...
class Protocol
  ...
  def find_batch(plasmid_items)
  	...
  	ecoli_batch.each do |item|
    	if plasmid_items.length == 1 and plasmid_items[0].sample.name == "SSJ128" and Item.find(item.id).get("tested") == "No"
    		return item
    	elsif plasmid_items.length != 1 and Item.find(item.id).get("tested") == "Yes"
    		return item
    	end
    end
  end
