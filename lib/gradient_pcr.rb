module GradientPCR
  def distribute_pcrs fragment_info_list, num_therm
    ...
    frags_by_bins.reject { |frag_hash| frag_hash[:rows].empty? }.map do |frag_hash|
      { 
        fragment_info: Hash[frag_hash[:rows].sort], bins: frag_hash[:bins], mm: 0, ss: 0, fragments: [], templates: [], forward_primers: [],
  ...
  def sort_temperatures_into_bins an_temps, num_therm
    ...
    best_bin_set = find_best_bin_set an_temps, bins, (50..62).map { |t| t / 1 }, Array.new, num_therm
