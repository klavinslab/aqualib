      ...
      if primer_ids.all? { |i| i != 0 }
        ...
        if num_of_overnights_started
          if num_of_overnights_started < 2 && num_colony >= num_of_overnights_started
            ...
            t = Task.new(name: "#{plate.sample.name}_plate_#{plate_id}_retry", specification: { "plate_ids E coli Plate of Plasmid" => [plate.id], "num_colonies" => [1], "primer_ids Primer" => [primer_ids], "initials" => "" }.to_json, task_prototype_id: tp.id, status: "waiting", user_id: plate.sample.user.id)
            t.save
